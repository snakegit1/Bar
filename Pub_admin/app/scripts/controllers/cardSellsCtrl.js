 ...
 */
angular.module('yapp')
  .controller('cardSellsCtrl', function($scope, $rootScope, $state, $stateParams, $auth, $resource, $location, User, Pub, $uibModal, cardSellsSrv) {
      ...
      cardSellsSrv.query({pub_id: $stateParams.id, date_start: $scope.date_start, date_end: $scope.date_end}).$promise.then(function (results) {
            ...
            for(var i in results) {
              ...
              for(var y in results[i].drinks) {
                // Fields
                var body = [];
                body.push( results[i].id, results[i].pub.name, results[i].drinks[y].name, results[i].status_traslated, results[i].created_at_formated, results[i].updated_at_formated, results[i].drinks[y].price, results[i].drinks[y].quantity, results[i].drinks[y].price * results[i].drinks[y].quantity);
      ...
      if($("#display-order-" + order_id).length == 0) {
        ...
        cardSellsSrv.query({id: order_id}).$promise.then(function (results) {
          ...
          $scope.payment = results[0].payment;
            ...
            buffer += '<td colspan="7" style="background-color:#666">';
